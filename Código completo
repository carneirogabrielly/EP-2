...
#função que recebe as informações de onde posicionar o navio e devolve as posições que ele irá ocupar
...
#função que preenche o dicionario frota com as posições ocupadas por cada navio
...
#função que muda o símbolo do tabuleiro depois de cada jogada
...
#função que posiciona os navios da frota no tabuleiro
...
#funcão que contabiliza quantos navios ja foram afundados
...
#cria o dicionario vazio frota
frota = {}

#cria o dicionario com a quantidade de cada embarcação e o tamanho delas
...
#vai perguntar para o jogador onde ele quer colocar cada tipo de navio
...
while jogando:
    ...
    while valida_posicao:
        ...
        #função para o computador escolher uma posição aleatória para atacar
            ...
            #Se a posição já foi atacada ele vai continuar sorteando
            while [sortear_linha, sortear_coluna] in posicoes_oponente_atacou:
                sortear_linha = random.randint(0,9)
                sortear_coluna = random.randint(0,9)

            #se a posição nao foi atacada ele ataca e atualiza o tabuleiro do jogador
            ...
            #verifica se o oponente já atacou todos os navios
